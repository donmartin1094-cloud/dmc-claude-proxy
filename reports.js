...
function _qcFileRowHtml(r, canManage) {
  ...
  return '<div class="qc-file-row">' +
    ...
    '</div>' +
    '<button onclick="_qcPreviewFile(\'' + escHtml(r.id) + '\')" style="background:none;border:1px solid rgba(167,139,250,0.4);color:#a78bfa;font-family:\'DM Mono\',monospace;font-size:10px;padding:2px 8px;border-radius:4px;cursor:pointer;flex-shrink:0;">👁 Preview</button>' +
...
function _qcPreviewFile(recId) {
  var r = (typeof qcReports !== 'undefined' ? qcReports : []).find(function(x) { return x.id === recId; });
...
// Reads the current schedule slot for a stub — foreman name, job #/name/GC,
// and plant — fresh at render time so an auto-generated stub always reflects
// whatever is on the schedule right now, rather than what was true when it
// was created. foremanHint is used only to resolve which slot ('top' vs
// 'bottom') the stub was originally tied to, matching the same name-based
// convention _invGetSchedFields() already uses elsewhere in this file.
function _inv3LiveStubFields(dateKey, foremanHint) {
  var _slot = (foremanHint||'').toLowerCase().indexOf('filipe') !== -1 ? 'top' : 'bottom';
  var _day = (typeof schedData !== 'undefined' ? schedData[dateKey] : null) || {};
  var _slotObj = _day[_slot] || {};
  var _f = _slotObj.fields || {};
  var _liveForeman = _f.foreman || _f.crew
    || (typeof foremanRoster !== 'undefined' && foremanRoster[_slot === 'top' ? 0 : 1])
    || foremanHint || '';
  var _bj = (typeof backlogJobs !== 'undefined' ? backlogJobs : []).find(function(j){
    return (j.num||'').toString().trim() === (_f.jobNum||'').toString().trim();
  });
  var _gcName = _f.gcName || _f.owner || (_bj && _bj.gc) || '';
  return {
    slot: _slot,
    foreman: _liveForeman,
    jobNo: _f.jobNum || '',
    jobName: _f.jobName || '',
    gcName: _gcName,
    supplier: _f.plant || ''
...
function _inv3DayBlock(dateKey, report, invoice, canEdit) {
  // A stub (auto-generated, no submitted report attached, never manually
  // corrected) always reads its display fields fresh from the live schedule
  // so it reflects whatever is currently scheduled. A real invoice
  // (autoGenerated:false) or one an admin has manually edited keeps its
  // saved values as-is — those are authoritative and must not be overwritten.
  var _isLiveStub = !report && invoice && invoice.autoGenerated && !invoice.manuallyEdited;
  var _liveFields = _isLiveStub ? _inv3LiveStubFields(dateKey, invoice.foreman) : null;

  var foremanName = _liveFields ? _liveFields.foreman : (report ? report.foreman : (invoice ? invoice.foreman : ''));
  var initials = _inv3Initials(foremanName);
  var jobNo    = escHtml(_liveFields ? (_liveFields.jobNo || '—') : (report ? (report.jobNumber || '—') : (invoice ? (invoice.jobNo || '—') : '—')));
  ...
  // Supplier — computed for both the filled and empty/stub render paths (using
  // dateKey/foremanName rather than invoice.dateOfWork/invoice.foreman so this
  // still works when invoice is null) so placeholder blocks show it too.
  var _fBlk = _invGetSchedFields(dateKey, foremanName);
  var _supplier = _liveFields ? (_liveFields.supplier || '') : ((invoice && invoice.supplier) || (_fBlk && _fBlk.plant) || '');
  ...
  if (isEmpty) {
    ...
    var _stubSlot = (foremanName||'').toLowerCase().indexOf('filipe') !== -1 ? 'top' : 'bottom';
    var _editBtn = (typeof isAdmin === 'function' && isAdmin())
      ? '<button onclick="event.stopPropagation();_inv3EditStub(\''+escHtml(dateKey)+'\',\''+_stubSlot+'\',\''+escHtml(foremanName)+'\')" title="Edit stub" style="position:absolute;top:2px;right:2px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.2);border-radius:3px;color:var(--concrete-dim);font-size:9px;padding:1px 4px;cursor:pointer;line-height:1;">✏️</button>'
      : '';
    var _emptyStyleParts = ['position:relative;'];
    if (!_matchesFilter) _emptyStyleParts.push('opacity:0.25;');
    return '<div class="inv3-day-block empty" style="'+_emptyStyleParts.join('')+'"'+(handler?' onclick="'+handler+'"':'')+' title="'+escHtml(foremanName)+'">'
      + _editBtn
...
// Lightweight modal for correcting a stub's job info without opening the
// full invoice modal. Prefills from an existing manually-edited stub for
// this date+foreman (if one exists) so re-editing doesn't lose prior edits,
// otherwise from the live schedule.
function _inv3EditStub(dateKey, slot, foremanName) {
  document.getElementById('_inv3EditStubOverlay')?.remove();
  var _day = (typeof schedData !== 'undefined' ? schedData[dateKey] : null) || {};
  var _f = (_day[slot] || {}).fields || {};
  var _existing = (invoiceList||[]).find(function(i){
    return i.dateOfWork===dateKey && (i.foreman||'').toLowerCase()===(foremanName||'').toLowerCase() && i.autoGenerated;
  });
  ...
  var _jobNum  = (_existing && _existing.jobNo)   || _f.jobNum  || '';
  var _jobName = (_existing && _existing.jobName) || _f.jobName || '';
  var _gcName  = (_existing && _existing.gcName)  || _f.gcName || (_bj && _bj.gc) || '';
  var _plant   = (_existing && _existing.supplier)|| _f.plant  || '';

  var _dateLabel = new Date(dateKey+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});

  var overlay = document.createElement('div');
  overlay.id = '_inv3EditStubOverlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:9700;display:flex;align-items:center;justify-content:center;padding:20px;';
  overlay.innerHTML =
    '<div style="background:var(--asphalt-mid);border:1px solid var(--asphalt-light);border-radius:var(--radius-lg);width:100%;max-width:420px;padding:20px;">'+
      '<div style="font-family:\'Bebas Neue\',sans-serif;font-size:18px;letter-spacing:1.5px;color:var(--stripe);margin-bottom:2px;">Edit Stub — '+escHtml(_dateLabel)+'</div>'+
      '<div style="font-family:\'DM Mono\',monospace;font-size:10px;color:var(--concrete-dim);margin-bottom:14px;">Foreman: '+escHtml(foremanName)+'</div>'+
      '<div style="display:flex;flex-direction:column;gap:10px;">'+
        '<div><label class="inv-form-label">Job #</label><input class="inv-input" id="_stubJobNum" value="'+escHtml(_jobNum)+'" style="width:100%;box-sizing:border-box;"/></div>'+
        '<div><label class="inv-form-label">Job Name</label><input class="inv-input" id="_stubJobName" value="'+escHtml(_jobName)+'" style="width:100%;box-sizing:border-box;"/></div>'+
        '<div><label class="inv-form-label">GC</label><input class="inv-input" id="_stubGcName" value="'+escHtml(_gcName)+'" style="width:100%;box-sizing:border-box;"/></div>'+
        '<div><label class="inv-form-label">Supplier/Plant</label><input class="inv-input" id="_stubPlant" value="'+escHtml(_plant)+'" style="width:100%;box-sizing:border-box;"/></div>'+
      '</div>'+
      '<div style="display:flex;gap:8px;margin-top:16px;">'+
        (_existing ? '<button onclick="_inv3DeleteStub(\''+escHtml(dateKey)+'\',\''+escHtml(slot)+'\',\''+escHtml(foremanName)+'\')" style="padding:8px 12px;background:rgba(232,89,89,0.15);border:1px solid #e85555;border-radius:var(--radius);color:#e85555;font-family:\'DM Mono\',monospace;font-size:11px;cursor:pointer;">🗑️ Delete</button>' : '')+
        '<button onclick="document.getElementById(\'_inv3EditStubOverlay\').remove()" style="flex:1;padding:8px;background:none;border:1px solid var(--asphalt-light);border-radius:var(--radius);color:var(--concrete-dim);font-family:\'DM Mono\',monospace;font-size:11px;cursor:pointer;">Cancel</button>'+
        '<button onclick="_inv3SaveStubEdit(\''+escHtml(dateKey)+'\',\''+escHtml(slot)+'\',\''+escHtml(foremanName)+'\')" style="flex:2;padding:8px;background:rgba(126,203,143,0.15);border:1px solid rgba(126,203,143,0.4);border-radius:var(--radius);color:#7ecb8f;font-family:\'DM Mono\',monospace;font-size:11px;font-weight:700;cursor:pointer;">💾 Save</button>'+
      '</div>'+
    '</div>';
  document.body.appendChild(overlay);
}
...
// Upserts the stub — matches an existing auto-generated stub for this
// date+foreman first (so this doesn't create a duplicate calendar block
// alongside one _inv3OnVerify()/the foreman report flow already created),
// falling back to a new deterministic id when nothing exists yet. Existing
// mixItems/trucking/approval data is preserved so this lightweight edit
// never wipes out billing info already entered via the full invoice modal.
function _inv3SaveStubEdit(dateKey, slot, foremanName) {
  var _jobNum  = (document.getElementById('_stubJobNum')  || {}).value || '';
  var _jobName = (document.getElementById('_stubJobName') || {}).value || '';
  var _gcName  = (document.getElementById('_stubGcName')  || {}).value || '';
  var _plant   = (document.getElementById('_stubPlant')   || {}).value || '';

  var _existingIdx = invoiceList.findIndex(function(i){
    return i.dateOfWork===dateKey && (i.foreman||'').toLowerCase()===(foremanName||'').toLowerCase() && i.autoGenerated;
  });
  var _existing = _existingIdx >= 0 ? invoiceList[_existingIdx] : null;

  var _stub = {
    id: _existing ? _existing.id : ('inv_stub_' + dateKey + '_' + slot),
    dateOfWork: dateKey,
    foreman: foremanName,
    jobNo: _jobNum,
    jobName: _jobName,
    gcName: _gcName,
    supplier: _plant,
    invoiceNo: (_existing && _existing.invoiceNo) || '',
    mixItems: (_existing && _existing.mixItems) || [],
    actualTrucking: (_existing && _existing.actualTrucking) || { dmcCount:0, dmcCost:0, brkRows:[], supCount:0, supCost:0 },
    approvedAmount: (_existing && _existing.approvedAmount) || '',
    invoiceNotes: (_existing && _existing.invoiceNotes) || '',
    attachments: (_existing && _existing.attachments) || [],
    billedTotal: (_existing && _existing.billedTotal) || 0,
    approved: (_existing && _existing.approved) || false,
    autoGenerated: true,
    manuallyEdited: true,
    status: (_existing && _existing.status) || 'pending',
    printed: (_existing && _existing.printed) || false,
    createdAt: (_existing && _existing.createdAt) || Date.now(),
    updatedAt: Date.now()
  };
...
function invShowQCModal(jobNum, date, qcLabel, event) {
  ...
  var _c = '<div style="background:#1a1f2e;border:1px solid rgba(59,130,246,0.4);border-radius:12px;width:95vw;max-width:1000px;height:90vh;display:flex;flex-direction:column;overflow:hidden;">';
  ...
  if (_qcFile) {
    ...
    _c += '<div style="flex:1;min-height:0;display:flex;flex-direction:column;padding:12px 16px 8px;">';
    _c += '<iframe src="https://docs.google.com/viewer?url=' + _enc + '&embedded=true" style="flex:1;min-height:0;width:100%;height:100%;border:none;border-radius:6px;background:#111;" allowfullscreen></iframe>';
